  ...
  useLayoutEffect(() => {
    ...
    const run = () => {
      ...
      ctx = gsap.context(() => {
        ...
        const topChars: HTMLElement[] = [];
        const bottomChars: HTMLElement[] = [];
        ...
        blocks.forEach((block) => {
          ...
          topChars.push(...(splitTop.chars as HTMLElement[]));
          bottomChars.push(...(splitBottom.chars as HTMLElement[]));
          gsap.set(splitTop.chars, {
            scaleY: 1,
            transformOrigin: '50% 0%',
            willChange: 'transform',
          });
          gsap.set(splitBottom.chars, {
            scaleY: 0,
        ...
        // Keep marquee hidden until we explicitly start the first cycle.
        gsap.set(blocks, { autoAlpha: 0 });

        const resetSplitChars = () => {
          gsap.set(topChars, {
            scaleY: 1,
            y: 0,
            transformOrigin: '50% 0%',
        ...
        const SQUISH_DUR_BASE = 2;
        const SQUISH_GAP_BASE = 1;
        const SQUISH_END_BASE =
          SQUISH_DUR_BASE + SQUISH_GAP_BASE + SQUISH_DUR_BASE;
        ...
        const OPEN_START = t(SQUISH_END_BASE);
        ...
        const openSlide = (i: number, onComplete?: () => void) => {
          ...
          // Keep split-text squish timing aligned per slide (reset phase).
          marqueeTLs.forEach((tl) => {
            tl.pause(0);
            tl.progress(0);
            tl.restart(true);
          });
          ...
          gsap
            ...
            .to(halfTop, { top: 0, duration: OPEN_DUR }, OPEN_START)
            .to(halfBottom, { bottom: 0, duration: OPEN_DUR }, OPEN_START)
            .fromTo(
              video,
              {
        ...
        const openingTl = gsap
          .timeline()
          .add(() => {
            // Show marquee text a bit earlier than the first open animation.
            resetSplitChars();
            gsap.set(blocks, { autoAlpha: 1 });
          }, 1)
          .to(
            topRef.current,
            { yPercent: -100, delay: 0.3, duration: 1, ease: 'power3.inOut' },
            0,
          )
          .to(
            bottomRef.current,
      ...
      >
        ...
        >
          <video
            ...
            playsInline
            preload="auto"
            poster="/assets/images/kv.webp"
            src="/assets/video/video-01-new.mp4"
          />
        ...
        >
          <video
            ...
            playsInline
            preload="auto"
            poster="/assets/images/kv-02.webp"
            src="/assets/video/video-02-new.mp4"
          />
          <div className="absolute inset-0 z-40 flex items-center justify-center text-center text-white">
            <div>
              <h3 className="relative mt-[min(20px,20px+100vw*0)] text-[clamp(18px,17.415px+100vw*.0015,20px)] leading-[1em] tracking-[-.01em] uppercase">
        ...
        >
          <video
            ...
            playsInline
            preload="auto"
            poster="/assets/images/kv-03.webp"
            src="/assets/video/video-03-new.mp4"
          />
          <div className="absolute inset-0 z-40 flex items-center justify-center text-center text-white">
            <div>
              <h3 className="relative mt-[min(20px,20px+100vw*0)] text-[clamp(18px,17.415px+100vw*.0015,20px)] leading-[1em] tracking-[-.01em] uppercase">
              ...
              {Array.from({ length: 3 }).map((_, i) => (
                ...
                >
                  <div className="js-ribbon flex items-center px-[max(21px,21px+100vw*.03)]">
                    <div className="js-split-block relative font-sans text-[clamp(80px,36.725px+100vw*.1725,300px)] leading-none font-bold whitespace-nowrap opacity-0">
        ...
        >
          <div className="invisible flex items-center opacity-0">
            <div className="relative font-sans text-[clamp(80px,36.725px+100vw*.1725,300px)] leading-none font-bold whitespace-nowrap">
              <span className="inline-block">KIVO TALK</span>
            </div>
          </div>
