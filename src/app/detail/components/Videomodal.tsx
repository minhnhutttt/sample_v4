...
export const VideoModal = ({ src, isOpen, onClose }: VideoModalProps) => {
  ...
  return (
    <div className="fixed inset-0 z-99" role="dialog" aria-modal="true">
      ...
      <div
        className="absolute right-0 bottom-0 left-0 bg-black"
        style={{
          height: '50%',
      ...
      <div
        ...
        style={{
          opacity: active ? 1 : 0,
          transition: active ? 'opacity 0.35s ease 0.5s' : 'opacity 0.2s ease',
      ...
      >
        ...
        {/* Video player */}
        <div className="flex h-full w-full max-w-4xl items-center justify-center p-6">
          <video
            ref={videoRef}
            className="object-container aspect-1170/2532 h-full w-full"
            loop
            muted
            playsInline
            autoPlay
